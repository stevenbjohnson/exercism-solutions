...
export function monthRate(ratePerHour, discount) {
  let full_rate = dayRate(ratePerHour) * BILLABLE_DAYS
  return applyDiscount(full_rate, discount)
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  let discounted_rate = applyDiscount(dayRate(ratePerHour), discount)
  return Math.floor(budget / discounted_rate)
}
...
function applyDiscount(value, percentage) {
  return Math.ceil(value - value * percentage)
}
